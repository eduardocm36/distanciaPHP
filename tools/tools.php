...
$command
    ...
    ->setCode(function (InputInterface $input, OutputInterface $output) {
        ...
        foreach ($directories as $directory) {
            if ($directory == 'Domain') {
                //entities
                mkdir("$base/$directory/Entities", 0755, true);
                echo "\n$base/$directory/Entities/{$componentName}" . ucfirst($directory) . ".php";
                $model = $twig->load('Mnt/Domain/Entities/entities.twig');
                $mdl = $model->render([
                    'name' => ucfirst($componentName),
                    'class' => ucfirst($componentName) . ucfirst('Entities'),
                    'ns' => $ns,
                    'namespace' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . '\\' . ucfirst($directory) . '\\' . ucfirst('Entities'),
                ]);
                file_put_contents("$base/$directory/Entities/{$componentName}Entities.php", $mdl);
            ...
            if ($directory === 'Http') {
                ...
                $gr = $controller->render([
                    ...
                    'entities' => ($componentPath === '' ? '' : $componentPath . '\\') . $componentName . '\\Domain',
                ]);
        ...
        $archivo = __DIR__ . '../../app/cmd/Services/Endpoints.php';
        ...
        if (!file_exists($archivo)) {
            ...
            $io->warning(sprintf("Ocurrió un error al agregar '%s', El archivo Endpoints.php no existe!", $nuevaLinea));
        ...
        foreach ($lineas as $linea) {
            ...
            } elseif (strpos($linea, 'class Endpoints') !== false) {
